from getWords import wordleGuessList, wordleAnswerList, wordmasterGuessList, wordmasterAnswersList
import time
...
def play(game_rows, browser, possible_guesses, possible_answers, classic_wordle=True):
    ...
    for guess_number in range(5):
        ...
        for word_to_guess in words_to_consider:
            ...
            for possible_answer in narrowed_down_list:
                ...
                if tuple(evaluation) not in temp_eval_to_words_map:
                    temp_eval_to_words_map[tuple(evaluation)] = [
                        possible_answer]
                else:
                    temp_eval_to_words_map[tuple(evaluation)].append(
                        possible_answer)

            # metric we are trying to minimize
            biggest_possible_remaining_wordcount = max(
                [len(val) for val in temp_eval_to_words_map.values()])
        ...
        time.sleep(1)
        if classic_wordle:
            answer_evaluation = get_wordle_evaluation(
                chosen_word, game_rows[guess_number], browser)
        else:
            answer_evaluation = get_wordmaster_evaluation(
                chosen_word, game_rows[guess_number], browser)
...
def run_program():
    ...
    if classic_wordle:
        ...
        board = browser.execute_script(
            "return arguments[0].shadowRoot.getElementById('board')", game_app)
        game_rows = board.find_elements(By.TAG_NAME, 'game-row')

        play(game_rows, browser, wordleGuessList(),
             wordleAnswerList(), classic_wordle)
    else:
        ...
        for _ in range(num_games):
            # get game rows
            game_rows = np.array(browser.find_elements(
                By.TAG_NAME, 'span')).reshape(6, 5)

            # guess list and answer list is the same
            play(game_rows, browser, wordmasterGuessList(),
                 wordmasterAnswersList(), classic_wordle)
            ...
            browser.find_element(
                By.XPATH, '//button[text()="Play Again"]').click()
            time.sleep(1)
    keyboard.wait(start_button)
